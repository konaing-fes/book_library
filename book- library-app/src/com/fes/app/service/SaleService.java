...
public class SaleService {
	...
	public void Inset(SaleDTO saleDTO) {
		...
		String saleInsert = "Insert into sale (sale_date, sale_time, sale_tax values (?,?,?)" ;
		String detailInsert = "Insert into sale_detial (quantiry, unit_price, book_id, sale id,book_category_id, book_author_id) values(?,?,?,?,?,?)";
		...
		try (Connection conn= ConnectionManager.getConnection();
			...
			while(rs.next()) {
				...
				for (SaleDetial sd : detial) {
					...
					insertDetial.setInt(4, sd.getSale().getId());
					insertDetial.setInt(5, sd.getCategory().getId());
					insertDetial.setInt(6, sd.getAuthor().getId());
	...
	public void update(SaleDTO saleDTO) {
		...
		String insertSD = "Insert into sale_detial (quantity, unit_price, book_id, sale id,book_category_id, book_author_id) values(?,?,?,?,?,?)";
	...
	public List<SaleDetial> findSaleDetial(Category category, String bookName, LocalDate dateFrom, LocalDate dateTo){
		...
		String sql = "select c.name  category_name, b.name  book_name, a.name  "
				+ "author_name, s.tax  sele_tax, sd.id sd_id, sd.sale_id sale_id, "
				+ "sd.book_id book_id, sd.book_category_id book_category_id, "
				+ "sd.book_author_id book_author_id, sd.unit_price  unit_price, "
				+ "sd.quantity  quantity, sum(sd.quantity * sd.unit_price + s.tax) total "
